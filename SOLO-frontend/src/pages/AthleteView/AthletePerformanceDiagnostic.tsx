...
const AthletePerformanceDiagnostic: React.FC = () => {
  ...
  const handleRunDiagnostic = () => {
    if (selectedMedia !== null) {
      ...
      history.push('/athlete-performance-result-page', { selectedMedia: mediaItems[selectedMedia] });
    }
  };
  ...
  return (
    <IonPage>
      ...
      <IonGrid className='media-grid'>
        <IonRow className='media-row'>
          {mediaItems.map((item, index) => (
            <IonCol size="auto" key={index} onClick={() => handleMediaClick(index)}>
