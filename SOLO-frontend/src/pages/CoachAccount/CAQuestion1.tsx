...
const CAQuestion1: React.FC = () => {
  ...
  const initialCoachData = useMemo(() => ({
  ...
  }, [coachData, login, history]);

  const QuestionComponent: React.FC<{
    questions: { label: string; name: keyof typeof coachData }[];
    coachData: typeof coachData;
    affiliations: Affiliation[];
    handleChange: (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => void;
  }> = useCallback(({ questions, coachData, affiliations, handleChange }) => {
    return (
      <>
  ...
  const allAnswersFilled = useMemo(() => coachData.title !== '' && coachData.affiliationId !== '', [coachData]);
