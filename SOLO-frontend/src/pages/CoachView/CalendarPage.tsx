...
const CalendarPage: React.FC = () => {
  ...
  const [workouts, setWorkouts] = useState<any[]>([]); // To store fetched workouts
  ...
  const generateWorkout = () => {
    ...
    const isoPSTDate = pstDate.toISOString();
    setDueDate(isoPSTDate);
    setShowModal(true);
  };
  ...
  const fetchWorkoutsByDate = async (date: string) => {
    try {
      const workouts = await ApiService.getWorkoutsByDate(date);
  ...
  return (
    <IonPage>
      ...
      <IonContent fullscreen>
        <CalendarBar onDateClick={fetchWorkoutsByDate} />
